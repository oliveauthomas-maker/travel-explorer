...
function Card({ item, color }) {
  return (
    <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "16px 18px", marginBottom: 8, position: "relative", overflow: "hidden" }}>
...
function LocationSearch({ onSelect }) {
  ...
  const pick = (feature) => {
    ...
    onSelect({ lat: coords[1], lng: coords[0] }, p.name || p.city || label);
  };
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, padding: "11px 15px" }}>
        ...
        {query && (
          <button onClick={() => { setQuery(""); setSuggestions([]); setOpen(false); }}
            style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", cursor: "pointer", fontSize: 18, padding: 0 }}>x</button>
        )}
      </div>
      {open && (
...
export default function App() {
  ...
  const filterByRadius = (items, rad) => {
    if (!location || !items || !items.length) return items || [];
  ...
  const hasResults = Object.values(data).some(v => v && v.length);
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          setCityName(d.address && (d.address.city || d.address.town || d.address.village) || (lat.toFixed(2) + ", " + lng.toFixed(2)));
        } catch { setCityName(lat.toFixed(3) + ", " + lng.toFixed(3)); }
        setLocLoading(false);
      },
      () => { setLocError("Position refusée."); setLocLoading(false); },
      { timeout: 10000 }
    );
  };
  ...
  const fetchData = async () => {
    ...
    const prompt = "You are a local travel expert. GPS: " + location.lat.toFixed(4) + ", " + location.lng.toFixed(4) + " (" + cityName + "). Radius: 40km. Search ALL towns within radius.\n\nReturn ONLY this JSON structure with no extra text:\n{\"activities\":[{\"title\":\"Name\",\"description\":\"Brief family-friendly description.\",\"lat\":0.0,\"lng\":0.0}],\"events\":[{\"title\":\"Nom\",\"description\":\"Description.\",\"lat\":0.0,\"lng\":0.0}],\"walks\":[{\"title\":\"Name\",\"description\":\"Easy walk max 15km.\",\"lat\":0.0,\"lng\":0.0}],\"vintage\":[{\"title\":\"Shop name\",\"description\":\"Thrift or vintage shop.\",\"lat\":0.0,\"lng\":0.0}]}\n\nUp to 5 items per category. Keep descripions under 10 words. For events: ONLY include events happening between " + dateStr + " and " + in2weeksStr + ". No past events, no events beyond 2 weeks. Always respond in French. Real places with accurate coordinates.";
    ...
    try {
      ...
      if (json.success && json.data) {
        ...
        const firstWithData = TABS.find(t => parsed[t.id] && parsed[t.id].length > 0);
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 500, margin: "0 auto", padding: "0 0 80px" }}>
        ...
        <div style={{ padding: "48px 24px 20px" }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
            {[["auto", "GPS"], ["manual", "Manuel"]].map(function(item) {
              const mode = item[0];
              const label = item[1];
              return (
            ...
            <div style={{ padding: "0 24px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                ...
                <p style={{ fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.4)", letterSpacing: 1, textTransform: "uppercase" }}>
                  {(data[activeTab] || []).length} résultat{(data[activeTab] || []).length > 1 ? "s" : ""} · {radius} km
                </p>
              </div>
